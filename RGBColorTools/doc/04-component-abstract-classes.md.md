...
- **Due Date**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->3/22 @11:59PM EST
...
1. I add getColor(). By introducing the getRGB method, it is possible to return an array of colors. I also added setRGB(), which allows the user to set the color.

2. I added IsValidColor() to determine if a color value is valid. With this method, I don't have to judge every method. You can ensure that the RGBColorTool instance is always in a valid state.

3. In the feedback of the last part, I changed the return type of convertColor (). Will return a new instance.
4. At the same time, I modified some parameters compared to the previous method. It makes it look clearer.
