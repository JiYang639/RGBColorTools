...
- **Due Date**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->3/1 @23:59 PM EST
...
I chose to go with the first component design from my part1, but I modified it a bit. Here is the modified RGB color pattern function. I chose this one for several reasons.
First, I think this component is easy and practical for clients to use, clients can use these methods to get what they want.
Second, it is easy to implement because the color can be represented by a list with length 3, and each integer in the list represents a different meaning. We can achieve the goal by calculating and operating each integer in the list.
Third, I am interested in designing, so it is practical for me. I will be happy if this component finally comes out.

- Component Design: RGB color pattern function
  - **Description**:  The main purpose of this component is to allow the user to be able to invoke different methods to have a precise definition of a color with RGB model (e.g., warm/cold, greyscale, etc.) and develop different color schemes based on these definitions, or to be able to get entirely new colors by overlaying colors.
  - **Kernel Methods**: 
	  -`int getR()`: gets the R-coordinate of `this`.
	  - `int getG()`: gets the G-coordinate of `this`.
	  - `int getB()` : gets the B-coordinate of `this`.
      - `int[] getList(int r,int g, int b)`: get RGB in sequence of `this`.
  - **Secondary Methods**:
	  - `boolean ColorIsWarm(int[] color)`: check if the color `this` is warm or cold.
	  - `int[] invertColor(int[] color)`: get the invert color RGB of the given color `this`.
	  - `int toGrayscale(int[] color)`: convert `this` to Grayscale.
	  - `int[] int[] blendColors(int[] color1, int[] color2, double ratio)`: get the blend colors of `this` and another given color.
	  - `int extractSingleChannel(int[] color, int Whichchannel)`: Extracting the single channel of `this`.  Whichchannel=1 (Extract Red); Whichchannel=2 (Extract Green); Whichchannel=3 (Extract Blue).
	  - `int[] overlayColors(int[] color1, int[] color2)`: Get the overlay Colors of `this` and another input color.
	  - `double colorSimilarity(int[] color1, List<int> color2)`: Get the similarity of two colors: `this` and another input color. Return 1 is exactly the same, and 0 is exactly different.
