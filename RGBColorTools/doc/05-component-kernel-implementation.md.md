...
- **Due Date**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->4/5 @11:59PM EST
...
1.  Using three components that correspond directly to the RGB color model is a very simple and convenient representation because it maps directly to the way we represent color on digital devices.  
    
2. Simplicity: This representation is clean and simple, easy to implement and maintain, and also facilitates color value manipulation and conversion, such as color blending, brightness adjustment, etc.

Representing RGB colors as a list of integers can help me model colors in a way that allows for calculating and operating over color values.


### **Convention**  

- The value of each color component (red, green, blue) must be an integer between 0 and 255, including both 0 and 255.
- The length of the list must be exactly 3, no more and no less, corresponding to each of the three RGB color components.  
    

### **Correspondence**  
The first element of the list (or the **`red`**  attribute of the class) corresponds to the red component of the color, the second element (or the **`green`** attribute) corresponds to the green component, and the third element (or the **`blue`** attribute) corresponds to the blue component.
